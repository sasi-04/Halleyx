    ...
    <Shell>
      <div className={`mb-6 p-4 rounded-2xl bg-gradient-to-r ${roleAccent} border`}>
        ...
        <p className="text-[11px] text-slate-400 mt-1">
          ...
          {role === "MANAGER" && "Approve expense requests from your team. Check your inbox."}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {(isManager || isAdmin) && (managerPending?.items?.length ?? 0) > 0 && (
        ...
        {(isManager || isAdmin) && (managerHistory?.items?.length ?? 0) > 0 && (
          <div className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-4 space-y-2 lg:col-span-2">
            <div className="flex items-center justify-between">
        ...
        {(isApprover || isFinance) && !isEmployee && !isAdmin && (
