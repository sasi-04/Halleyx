...
export class WorkflowEngine {
  ...
  static async processWorkflowStep(
    ...
    if (action === 'approve') {
      ...
      // Email notifications are handled by `EmailService` in `ExecutionService`.
      // Keep legacy workflow engine side-effect free to avoid duplicate/undefined SMTP wiring.
    }
  ...
  private static getWorkflowSteps(requestType: RequestType): WorkflowStep[] {
    if (requestType === RequestType.expense) {
      return [
        { step: 1, name: 'Employee Request', approver: null, status: RequestStatus.completed },
