...
async function main() {
  ...
  // Primary manager account used for approvals + email notifications
  await upsertUser({
    name: "Manager User",
  ...
  await upsertUser({ name: "HR", email: "hr@test.com", password: "hr123456", role: UserRole.HR });
  await upsertUser({ name: "CEO", email: "ceo@test.com", password: "ceo123456", role: UserRole.CEO });
