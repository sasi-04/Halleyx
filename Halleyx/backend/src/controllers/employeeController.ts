...
export async function createExpenseRequest(req: AuthedRequest, res: Response) {
  try {
    ...
    console.log("[employee] Request received", { type: "expense", userId });
    ...
    const requestService = serviceRegistry.getRequestService();
    const request = await requestService.createAndSubmit(RequestType.expense, workflow.id, userId, {
      employee_id: user.id,
    ...
    console.log("[employee] Request saved + workflow started", { requestId: request?.id, executionId: request?.execution_id });
    ...
    res.status(StatusCodes.CREATED).json({
      message: "Expense request created and sent to manager for approval",
      request
    });
  } catch (err: any) {
...
export async function createOnboardingRequest(req: AuthedRequest, res: Response) {
  try {
    const userId = req.user?.sub;
    if (!userId) return res.status(StatusCodes.UNAUTHORIZED).json({ error: "Not authenticated" });
    console.log("[employee] Request received", { type: "onboarding", userId });
    ...
    // Find or create onboarding workflow
    let workflow = await prisma.workflow.findFirst({
    ...
    const requestService = serviceRegistry.getRequestService();
    const request = await requestService.createAndSubmit(RequestType.onboarding, workflow.id, userId, {
      employee_id: user.id,
      employee_name: user.name,
      employeeId: user.id,
      employeeName: user.name,
      request_type: "onboarding",
      title: `Onboarding: ${newEmployeeName}`,
      newEmployeeName,
      email,
      position,
      department,
